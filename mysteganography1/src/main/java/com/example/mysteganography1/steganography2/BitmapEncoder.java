...
public class BitmapEncoder {
  ...
  public static byte[] decode(Bitmap inBitmap) {
    ...
    int len = 0;
    if ((byte) 0x5B == header[0] && (byte) 0x5B == header[1] &&
            (byte) 0x5D == header[HEADER_SIZE-2] &&(byte) 0x5D == header[HEADER_SIZE-1] ){
        len = (int) bytesToLong(Arrays.copyOfRange(header, 2, HEADER_SIZE - 2));
    }
